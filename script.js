...
class ElectricityMeterTracker {
    ...
    saveReadings() {
        ...
        this.showMessage('Daily usage data saved successfully! Previous values and current inputs unchanged.', 'success');
    }
    ...
    updatePreviousReadings() {
        ...
        if (confirm('This will update the previous reading values with current readings. This action affects all future calculations. Continue?')) {
            ...
            this.showMessage('Previous reading values updated successfully!', 'success');
        }
    }
    ...
    deleteRecord(index) {
        ...
        if (confirm(`Are you sure you want to delete the record from ${recordDate}?\n\nThis action cannot be undone.`)) {
            ...
            this.showMessage(`Record from ${recordDate} deleted successfully!`, 'success');
        }
    }
    ...
    exportData() {
        ...
        // Create CSV content
        let csvContent = 'Date,Import Peak Usage,Import Off Peak Usage,Export Peak Usage,Export Off Peak Usage,Net Usage\n';
        
        this.historicalData.forEach(record => {
            csvContent += `${record.date},${record.totalImportPeak || 0},${record.totalImportOffPeak || 0},${record.totalExportPeak || 0},${record.totalExportOffPeak || 0},${record.netUsage}\n`;
        });
        ...
        a.href = url;
        a.download = `electricity_meter_data_${new Date().toISOString().split('T')[0]}.csv`;
